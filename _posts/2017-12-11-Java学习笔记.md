...
9. 设置log4j的配置文件路径，在程序启动的时候，添加如下代码

   ```java
    PropertyConfigurator.configure("c:\log4j.properties");
   ```

10. [Spring mvc中controller默认也是单例的](https://blog.csdn.net/qq_27026603/article/details/67953879)，

11. ​

#### IDEA打包部署Maven项目时，resources文件没有拷贝到classess目录下，解决方法

解决方法一：需要在Idea中设置类路径，包含资源文件[intellij idea的classpath分析](https://blog.csdn.net/skyebefreeman/article/details/56495637)

解决方法二：如果方法一无效可以考虑[方法二:如何让properties或xml在java目录下一同被编译](https://blog.csdn.net/zsg88/article/details/71157632)

#### IDEA中Tomcat项目部署路径

[idea中tomcat文件部署路径](https://blog.csdn.net/joenqc/article/details/58044953)

#### lombok

1. `@Data`和`@EqualsAndHashCode(callSuper = true)`需要同时使用，原因参见[lombok @EqualsAndHashCode 注解的影响](https://blog.csdn.net/zhanlanmg/article/details/50392266)
2. ​
...
1. 视图渲染的路径有两种
   ...
   1. 绝对路径，比如下面的路径：
      ...
      整个语句表示webapp文件夹下面的aaa/bbb/ccc.html文件，和项目中的文件路径结构一致

   2. 相对路径，参考[Jfinal中的render](https://blog.csdn.net/u010087886/article/details/51006713)

#### JPA和JDBC的比较

1. [JPA or JDBC, how are they different?](https://stackoverflow.com/questions/11881548/jpa-or-jdbc-how-are-they-different)
...
3. [.net下的aop实现](https://blog.csdn.net/niuyongjie/article/details/1288448)，区别生命周期和AOP，生命周期是纵向的，AOP则是横向的
4. AOP在实际中最常见的应用：
   1. 在请特定的资源之前，添加权限验证
...
1. [SpringBoot中使用AOP](http://blog.didispace.com/springbootaoplog/)，其中使用了[pointcut的表达式](https://blog.csdn.net/elim168/article/details/78150438)

2. [使用AOP进行异常处理](https://www.tejusparikh.com/2011/logging-standard-exceptions-with-spring.html)，如果方法中有try-catch语句块，aop的异常处理捕获不到方法内部抛出的异常

3. SpringBoot配置文件：通过`spring.profiles.active`来指定当前需要的配置文件，[Profile-specific properties](https://docs.spring.io/spring-boot/docs/current/reference/html/boot-features-external-config.html#boot-features-external-config-profile-specific-properties)，在命令行中指定运行的配置文件：[Profiles](https://docs.spring.io/spring-boot/docs/current/reference/html/boot-features-profiles.html#boot-features-profiles)，在命令行中通过传递`--spring.profiles.active=dev`

4. 各种starter的作用[Spring Boot：定制自己的starter](http://www.jianshu.com/p/85460c1d835a)

5. JdbcTemplate的默认FetchSize大小是100，可以通过setFetchSize方法来设置，能够提高性能，减少数据连接次数[make your java run faster](http://makejavafaster.blogspot.com/2015/06/jdbc-fetch-size-performance.html)或者[JDBC performance tuning with optimal fetch size](https://venkatsadasivam.com/2009/02/01/jdbc-performance-tuning-with-optimal-fetch-size/)

6. 写Statement和写Sql来查询数据的区别
...
7. [Statement和Preparedstatement的区别](https://stackoverflow.com/questions/3271249/difference-between-statement-and-preparedstatement)

8. throws clause用途：不捕获异常，传递异常

9. `Class path contains multiple SLF4J bindings`的错误解决[Maven SLF4J: Class path contains multiple SLF4J bindings](https://stackoverflow.com/questions/22896243/maven-slf4j-class-path-contains-multiple-slf4j-bindings/26283350)

10. Test应用程序[Spring Boot features Testing](https://docs.spring.io/spring-boot/docs/current/reference/html/boot-features-testing.html) 

11. PreparedStatement的性能[PreparedStatement Performance](http://tutorials.jenkov.com/jdbc/preparedstatement.html#preparedstatement-performance)

12. PreparedStatement的查询需要next方法来读取数据

13. 关闭SpringBoot命令行进程
...
14. [Spring Boot使用Druid和监控配置](http://blog.csdn.net/xiaoyu411502/article/details/51392237)

15. [SpringBoot log4j2的RollingFile设置，每天生成新文件](https://www.boraji.com/log4j-2-rollingfileappender-example)
...
16. log4j2的[日志级别](https://logging.apache.org/log4j/2.x/manual/customloglevels.html)或者参照：[log4j2配置文件](http://www.cnblogs.com/hafiz/p/6170702.html)
...
17. [SpringBoot之CommandLineRunner接口和ApplicationRunner接口](http://blog.csdn.net/zknxx/article/details/52196427)

18. [CommandLineRunner和ApplicationRunner的区别和使用](https://dzone.com/articles/spring-boot-applicationrunner-and-commandlinerunne)，同时参考[Spring Boot CommandLineRunner and ApplicationRunner Example](https://www.concretepage.com/spring-boot/spring-boot-commandlinerunner-and-applicationrunner-example#commandlinerunner)

19. SpringBoot添加命令行参数：[config properties management](https://docs.spring.io/spring-boot/docs/current/reference/html/boot-features-external-config.html)

20. 创建一个可运行的jar，[fully executable](https://docs.spring.io/spring-boot/docs/current/reference/html/deployment-install.html)并且部署到服务器

21. 自动退出SpringBoot控制台程序，[Spring Boot - Application exit code](http://www.logicbig.com/tutorials/spring-framework/spring-boot/app-exit-code/)
...
22. `@Component`注解的意思[@Component, @Service, @Repository and @Controller](https://www.concretepage.com/spring/spring-auto-detection-with-component-service-repository-and-controller-stereotype-annotation-example-using-componentscan-and-component-scan#component)

23. `whitelabel error page`错误解决
...
24. 过滤器[添加自定义过滤器](http://www.baeldung.com/spring-security-custom-filter)，用户授权这类的操作，最好是在过滤器中操作，而不是添加拦截器

25. 拦截器[[Spring Boot 拦截器](http://blog.csdn.net/catoop/article/details/50501696)]和[Spring Boot 静态资源和拦截器处理](http://blog.csdn.net/gebitan505/article/details/70142025)

26. 拦截器实现[Springboot实现拦截器](https://www.cnblogs.com/jun-ma/p/5679459.html)

27. 拦截器和过滤器中如果添加`AuthenticationManager`的引用，有可能出现循环引用，最好的做法是创建拦截器的时候不再添加过滤器，只保持一个即可
...
28. [用户登录授权](https://hellokoding.com/registration-and-login-example-with-spring-security-spring-boot-spring-data-jpa-hsql-jsp/)

29. Java Web使用token认证的两种形式[jwt-token-authentication-with-spring-boot](http://www.svlada.com/jwt-token-authentication-with-spring-boot/#jwt-authentication)

30. [@Bean注解作用](http://blog.csdn.net/javaloveiphone/article/details/52182899)：注册bean对象

31. [@Autowired属性为null原因分析](https://stackoverflow.com/questions/19896870/why-is-my-spring-autowired-field-null)

32. "Could not autowire. No beans of "，解决方法如下：
...
33. 如何快速的定位被子类重写的父类方法

    1. 在idea中通过标记迅速定位到父类中的实现
    2. 定位到父类中的实现方法后，通过查找该方法的引用，找到该方法在父类的调用位置
    3. 下载该方法源码，一步一步分析实现

34. Spring中框架中方法链中的每个方法叫做End point，自己通过实现接口或者继承抽象类 对应的类中的 方法叫point cut。通过定义aop的实现，一定程度上减化了处理问题的难度，具体表现在很多抽象类的内部实现，大部分内容都是实现完成，我们只需要做的只是重新恰当的方法，业务流程甚至都可以直接沿用，不需要任何修改，因为很多类都已Null开头，表明这个方法也是很好的切入点，来实现自己的个性业务

35. **Spring Boot中的异常处理**，参考文档[[SpringBoot全局异常处理](http://blog.csdn.net/tianyaleixiaowu/article/details/70145251)]或者参考官方文档[Error Handling](https://docs.spring.io/spring-boot/docs/1.5.10.RELEASE/reference/htmlsingle/#boot-features-error-handling)

    1. 进入Controller前的全局异常处理
    2. Controller全局异常处理

36. Spring异常处理，[官网的说明](https://spring.io/blog/2013/11/01/exception-handling-in-spring-mvc)

37. @ControllerAdvice捕获不到未进入Controller时的错误，需要添加全局错误处理类[**Spring Boot**中ErrorController实现](https://blog.csdn.net/qianyiyiding/article/details/76691428)或者参考[全局错误处理](https://segmentfault.com/a/1190000008443705)

38. [There is no default constructor错误](http://blog.csdn.net/m0_38021517/article/details/73647707)

39. spring.factories文件

40. [class<T>和 class<?>类型 有什么区别](http://www.cnblogs.com/skyislimit/p/5665853.html)

41. 查找类源码中的具体指定类，利用intellij中的search everywhere功能

42. 构造函数上添加@Autowired作用
...
44. [java中Servlet Cookie取不到值原因解决办法](http://www.jb51.net/article/116520.htm)在测试带Cookie的HTTP请求时发现，服务端用request.getHeader("cookie")可以去到值; 但是用request.getCookies()却不行
...
45. @SpringBootApplication注解包含了 @Configuration @EnableAutoConfiguration @ComponentScan这三个注解，里面有@EnableAutoConfiguration and @ComponentScan里包含的属性

46. [“repackage failed: Unable to find main class” with Maven on spring-boot project](https://stackoverflow.com/questions/42937577/unable-to-find-main-class-with-maven-on-spring-boot-project-in-eclipse) 

    提示项目缺少入口类，这是因为自己把maven的build插件放到了父pom中，而service项目是没有main入口的。解决：把父pom中的build插件节点去掉，只在有main入口的项目的pom中

47. [Spring Boot部署到外部的tomcat服务器](https://blog.csdn.net/u010598360/article/details/78789197)

48. 在一些业务场景中，当容器初始化完成之后，需要处理一些操作，比如一些数据的加载、初始化缓存、特定任务的注册等等。这个时候我们就可以使用Spring提供的ApplicationListener来进行操作[Springboot中SpringApplicationEvent的种类](https://www.cnblogs.com/ssslinppp/p/7607509.html)

49. [Spring boot程序打包瘦身](https://www.cnblogs.com/yueli/p/7810796.html)
...
8. [Thymeleaf给javascript变量赋值](https://blog.csdn.net/mygzs/article/details/52667897)

9. [Thymeleaf获取cookie信息](https://blog.csdn.net/qq_28097737/article/details/78049269)
