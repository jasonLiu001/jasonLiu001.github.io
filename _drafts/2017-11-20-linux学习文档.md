...
24. #### Nginx实现tomcat负载
    ...
    2. 有2台或以上的机器配置，也很简单，只需要添加upstream节点，然后把proxy_pass的值指向该upstream节点对应的名称即可，**注意：1. 在upstream中没有http://协议头。 2. upstream名称中最好不要带下划线之类的 3. 名字可以包含点符号，最好是用对外暴露的域名名称，避免代码中获取服务器名称的时候有问题**有可能导致访问服务器失败，自己踩过的坑
       ...
       ```shell
           ...
           upstream localtomcat {
               ...
               location / { # / 代表匹配所有请求
                 proxy_pass  http://localtomcat;  # 对应upstream名称，建议填对应域名
               }
               .....省略无用的
           }
